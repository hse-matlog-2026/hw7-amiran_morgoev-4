...
class Model(Generic[T]):
    ...
    def evaluate_formula(self, formula: Formula,
        ...
        for value in self.universe:
            ...
            truth_value = self.evaluate_formula(formula.statement,
                                                frozendict(extended_assignment))
            if formula.root == 'A' and not truth_value:
                return False
            if formula.root == 'E' and truth_value:
                return True
    ...
    def is_model_of(self, formulas: AbstractSet[Formula]) -> bool:
        ...
        for formula in formulas:
            ...
            for values in product(self.universe, repeat=len(free_variables)):
                assignment = frozendict({variable: value for variable, value in
